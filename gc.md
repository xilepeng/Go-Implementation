...
1. [Go V1.5 三色并发标记法](#go-v15-三色并发标记法)
2. [屏障机制](#屏障机制)
3. [什么是 GC，有什么作用？](#什么是-gc有什么作用)
4. [根对象到底是什么？](#根对象到底是什么)
5. [常见的 GC 实现方式有哪些？Go 语言的 GC 使用的是什么？](#常见的-gc-实现方式有哪些go-语言的-gc-使用的是什么)
6. [三色标记法是什么？](#三色标记法是什么)
7. [STW 是什么意思？](#stw-是什么意思)
8. [如何观察 Go GC？](#如何观察-go-gc)
9. [有了 GC，为什么还会发生内存泄露？](#有了-gc为什么还会发生内存泄露)
10. [隔离屏障和混合写屏障到底有什么区别？](#隔离屏障和混合写屏障到底有什么区别)


## Go V1.5 三色并发标记法
        
1. 程序起初创建对象，对象全部标记为白色，将所有对象全部放入白色标记表中；
2. 遍历 RootSet 根节点集合，得到灰色节点，将灰色对象加入灰色标记表；
3. 遍历灰色标记表，将可达对象从白色标记为灰色，将遍历过的灰色标记为黑色；
4. 重复上一步，直到灰色标记表中无任何对象；（遍历灰色标记表，将可达对象从白色标记为灰色，将遍历过的灰色标记为黑色）
5. 回收所有白色标记表中对象（垃圾回收）
...
- 强三色不变式：不存在黑色对象引用到白色对象（强制性不允许黑色对象引用白色对象，解决白色对象被误删除的情况。）
...
- 满足：强三色不变式（不存在黑色对象引用白色对象，因为白色会强制变为灰色）
- 不足：结束时需要 STW 重新扫描栈，大约需要 10~100 ms 

伪代码
```go
func 添加下游对象(当前下游对象 slot, 新下游对象 ptr) {
    新下游对象 ptr 标记为灰色
...
堆：空间很大，提供的内存很多
栈：空间很小，不仅仅提供对象的内存，还提供日常函数的调用（压栈，出栈），为了不影响性能，插入屏障不在栈上使用


3. 删除屏障：对象被删除时触发

- 具体操作：被删除的对象，如果自身为灰色或白色，那么标记为灰色
- 满足：弱三色不变式（保护灰色对象到白色对象的路径不会断），保护一轮
- 不足：回收精度低，一个对象即使被删除了最后一个指向它的指针也依然可以活过这一轮，在下一轮GC中被删除

 
伪代码
```go
func 添加下游对象(当前下游对象 slot, 新下游对象 ptr) {
    if (当前下游对象 slot 是灰色 || 当前下游对象是白色) {
        当前下游对象 slot 标记为灰色
    }
    当前下游对象 slot = 新下游对象 ptr
}
...
具体操作：
1. GC 开始将栈上对象全部扫描并标记为黑色（之后不再进行第二次重复扫描，无需 STW)
